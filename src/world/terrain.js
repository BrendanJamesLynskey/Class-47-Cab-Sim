// terrain.js — the ground: fields, hills and the hedges and walls between them.
//
// Picture the land beside the track as strips ("bands") running away from the line:
//
//     far ... | band 3 | band 2 | band 1 | band 0 | fence | TRACK | fence | band 0 | band 1 ...
//
// Each band is cut along its length into fields of different sizes. Every field
// gets a "kind" (pasture, ploughed, wood ...) chosen with the seeded random numbers,
// so the countryside is different every few hundred metres but always the same on
// every run.

import * as C from "../config.js";
import { pointBeside } from "../path.js";
import { makeRandom, hashRandom } from "./random.js";
import { colour } from "./mesh-builder.js";

// How far each band starts from the middle of the track, in metres.
// (band 0 is EDGES[0]..EDGES[1], and so on.)
export const EDGES_M = [4.3, 30, 65, 115, 190, 320, 600, 950];
export const BAND_COUNT = EDGES_M.length - 1;
export const SIDES = [-1, 1]; // -1 = left of the track, +1 = right

const ROW_M = 20;              // the ground is built in rows this long
const STRIPES = 6;             // and each field is cut into this many stripes (ploughing, mowing)
const HILL_HEIGHT_M = 16;      // how high the rolling hills get
const TRACKBED_DROP_M = 0.65;  // the ground beside the track is this far below the rail
const HEDGE_EDGES = [1, 2, 3, 4]; // which lines between bands get hedges (the near ones only)

const smoothstep = (a, b, x) => {
  const t = Math.min(1, Math.max(0, (x - a) / (b - a)));
  return t * t * (3 - 2 * t);
};

// The rolling hills. They are flat next to the track and highest a couple of hundred
// metres out, then flatten off again far away so they join the horizon.
function hillAmount(offset_m) {
  const away = Math.abs(offset_m);
  return HILL_HEIGHT_M * smoothstep(25, 190, away) * (1 - smoothstep(450, 950, away));
}
function hillAtNode(nodeIndex, offset_m) {
  const wobble = 0.6 * Math.sin(nodeIndex * 0.37 + offset_m * 0.019) + 0.4 * Math.sin(nodeIndex * 0.91 - offset_m * 0.011 + 1.3);
  return hillAmount(offset_m) * (0.5 + 0.5 * wobble);
}

function pickKind(random, band, length_m) {
  const r = random();
  if (band === 1 && length_m >= 110 && r < 0.07) return "farm";
  if (band <= 3 && r < 0.14) return "wood";
  if (band <= 3 && r < 0.18) return "conifer";
  if (r < 0.31) return "ploughed";
  if (r < 0.66 && band <= 2) return "pasture";
  return "grass";
}

// Works out all the fields for the whole route, once, at start-up.
function makeLayout(seed, totalLength_m) {
  const runs = { [-1]: [], [1]: [] }; // runs[side][band] = list of fields
  for (const side of SIDES) {
    for (let band = 0; band < BAND_COUNT; band++) {
      const random = makeRandom(seed * 1000 + (side + 1) * 100 + band);
      const list = [];
      let start = -random() * 150;
      while (start < totalLength_m) {
        const length = 80 + band * 15 + random() * (120 + band * 40);
        const kind = pickKind(random, band, length);
        list.push({
          start, end: start + length, kind, side, band,
          colorIndex: Math.floor(random() * C.GRASS_COLORS.length),
          brightness: 0.92 + random() * 0.14,
        });
        start += length;
      }
      runs[side].push(list);
    }
  }
  return runs;
}

// What sits along the line between two fields: a hedge, a drystone wall or a fence.
export function boundaryType(seed, side, edgeOrBand, bucket) {
  const r = hashRandom(seed + 7, side * 10 + edgeOrBand, bucket);
  if (r < 0.66) return "hedge";
  if (r < 0.86) return "wall";
  return "fence";
}

// The boundary along the track on the line between two bands, at distance d.
// (Hedges here, and hedgerow trees in nature.js, both ask this so they agree.)
export function edgeBoundaryType(seed, side, edge, d) {
  return boundaryType(seed, side, 20 + edge, Math.floor(d / 180));
}

export function createTerrain(ctx) {
  const { path, seed } = ctx;
  const layout = makeLayout(seed, path.totalLength_m);
  const here = {};
  const there = {};

  // How high is the ground at this distance along the track and this far to the side?
  function groundY(d, offset_m) {
    const trackY = path.pathAt(d, here).y;
    const k = Math.floor(d / ROW_M);
    const t = d / ROW_M - k;
    const hill = hillAtNode(k, offset_m) * (1 - t) + hillAtNode(k + 1, offset_m) * t;
    return trackY - TRACKBED_DROP_M + hill;
  }

  // A point on the ground: [x, y, z] at distance d, offset_m to the side.
  function groundPoint(d, offset_m) {
    const p = path.pathAt(d, there);
    const side = pointBeside(p, offset_m, {});
    return [side.x, groundY(d, offset_m), side.z];
  }

  // Cuts [from, to] into pieces that don't cross a multiple of ROW_M.
  function rowsBetween(from, to) {
    const rows = [];
    let a = from;
    while (a < to - 1e-6) {
      const b = Math.min(to, (Math.floor(a / ROW_M + 1e-9) + 1) * ROW_M);
      rows.push([a, b]);
      a = b;
    }
    return rows;
  }

  function addFieldRow(builder, run, a, b, rowSeed) {
    const inner = EDGES_M[run.band] * run.side;
    const outer = EDGES_M[run.band + 1] * run.side;
    const base = colour(run.kind === "ploughed" ? C.PLOUGHED_COLOR : C.GRASS_COLORS[run.colorIndex]);
    const kindBrightness = run.kind === "wood" || run.kind === "conifer" ? 0.7 : 1;
    const rowBrightness = run.brightness * kindBrightness * (0.97 + 0.06 * hashRandom(seed, rowSeed, run.band));
    const stripeContrast = run.kind === "ploughed" ? 0.09 : 0.035;

    for (let s = 0; s < STRIPES; s++) {
      const o0 = inner + ((outer - inner) * s) / STRIPES;
      const o1 = inner + ((outer - inner) * (s + 1)) / STRIPES;
      const brightness = rowBrightness * (1 + (s % 2 === 0 ? stripeContrast : -stripeContrast));
      builder.addQuad(groundPoint(a, o0), groundPoint(a, o1), groundPoint(b, o1), groundPoint(b, o0), base, brightness);
    }
  }

  // The stony strip either side of the rails.
  function addTrackbed(builder, a, b) {
    const half = EDGES_M[0];
    const pa = path.pathAt(a, {}), pb = path.pathAt(b, {});
    const corner = (p, o) => { const s = pointBeside(p, o, {}); return [s.x, p.y - TRACKBED_DROP_M, s.z]; };
    builder.addQuad(corner(pa, -half), corner(pa, half), corner(pb, half), corner(pb, -half), colour(C.CESS_COLOR), 1);
  }

  // A hedge, wall or fence from point A to point B (each [x, y, z]).
  function addBoundary(builder, type, a, b, variation) {
    if (type === "hedge") {
      builder.addBeam(a[0], a[1] - 0.1, a[2], b[0], b[1] - 0.1, b[2], 1.2, 1.4 + variation * 0.5, colour(C.HEDGE_COLOR), 0.85 + variation * 0.3);
    } else if (type === "wall") {
      builder.addBeam(a[0], a[1] - 0.1, a[2], b[0], b[1] - 0.1, b[2], 0.55, 1.0, colour(C.WALL_COLOR), 0.9 + variation * 0.2);
    } else {
      // A post-and-rail fence: one rail at head height and a post at the start.
      builder.addBeam(a[0], a[1] + 0.85, a[2], b[0], b[1] + 0.85, b[2], 0.06, 0.09, colour(C.FENCE_COLOR));
      builder.addBox(a[0], a[1] + 0.5, a[2], 0.1, 1.05, 0.1, 0, colour(C.FENCE_COLOR));
    }
  }

  // Adds all the ground and field boundaries between distances d0 and d1.
  function addTerrain(builder, d0, d1) {
    for (const [a, b] of rowsBetween(d0, d1)) addTrackbed(builder, a, b);

    for (const side of SIDES) {
      for (let band = 0; band < BAND_COUNT; band++) {
        for (const run of layout[side][band]) {
          if (run.end <= d0 || run.start >= d1) continue;
          for (const [a, b] of rowsBetween(Math.max(run.start, d0), Math.min(run.end, d1))) {
            addFieldRow(builder, run, a, b, Math.floor(a));
          }
          // A hedge across the start of the field (only near ones).
          if (band <= 3 && run.start >= d0 && run.start < d1) {
            const type = boundaryType(seed, side, band, Math.floor(run.start));
            for (let s = 0; s < STRIPES; s++) {
              const o0 = EDGES_M[band] * side + ((EDGES_M[band + 1] - EDGES_M[band]) * side * s) / STRIPES;
              const o1 = EDGES_M[band] * side + ((EDGES_M[band + 1] - EDGES_M[band]) * side * (s + 1)) / STRIPES;
              addBoundary(builder, type, groundPoint(run.start, o0), groundPoint(run.start, o1), hashRandom(seed, Math.floor(run.start), s));
            }
          }
        }
      }
      // Hedges along the track between bands.
      for (const edge of HEDGE_EDGES) {
        for (const [a, b] of rowsBetween(d0, d1)) {
          const type = edgeBoundaryType(seed, side, edge, (a + b) / 2);
          addBoundary(builder, type, groundPoint(a, EDGES_M[edge] * side), groundPoint(b, EDGES_M[edge] * side), hashRandom(seed, Math.floor(a), edge));
        }
      }
    }
  }

  return { addTerrain, groundY, groundPoint, layout };
}
